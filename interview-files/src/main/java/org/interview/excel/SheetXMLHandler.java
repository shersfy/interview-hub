...
/**
 * Reference {@link XSSFSheetXMLHandler}, overide endElement() method.
...
public class SheetXMLHandler extends DefaultHandler {
	...
	public SheetXMLHandler(
			...
			DataFormatter dataFormatter,
			boolean formulasNotResults) {
		this(styles, null, strings, handler, dataFormatter, formulasNotResults);
	}
	...
	public SheetXMLHandler(
			...
			SheetContentsHandler handler,
			boolean formulasNotResults) {
		this(styles, strings, handler, new DataFormatter(), formulasNotResults);
	}
		...
		if (isTextTag(localName)) {
			...
			case NUMBER:
				...
				if (this.formatString != null && n.length() > 0){
					thisStr = ExcelReader.formatCellNumberData(formatter, n, this.formatString, this.formatIndex);
				}
				else{
					thisStr = n;
				}
